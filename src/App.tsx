...
type TagProps = { label: string; color?: string; bg?: string };

// ── Data ─────────────────────────────────────────────────────────────────────
const ROLES = [
  "CAD Design Engineer","PCB Design Engineer","Electrical Systems Engineer",
  "EV Engineer","Battery Engineer","Wiring Harness Engineer",
...
const DAILY_GUIDE = [
  { time: "8:00 – 8:30 AM", task: "LinkedIn Search", detail: "Search: 'EV Engineer fresher', 'CAD Design Engineer', 'Wiring Harness Engineer India'. Filter: Posted in last 24h. Easy Apply → Apply 4–5 jobs.", icon: "🔍" },
  { time: "8:30 – 9:00 AM", task: "Naukri.com Search", detail: "Login → Search same roles. Upload relevant resume PDF for each role. Apply 3–4 jobs. Enable 'Resume Highlight' for visibility.", icon: "📋" },
  { time: "9:00 – 9:20 AM", task: "Company Career Pages", detail: "Check: Ather, Ola Electric, Tata Elxsi, KPIT, Tata AutoComp, Motherson, Agnikul. Apply directly on their website — less competition.", icon: "🏢" },
  { time: "9:20 – 9:40 AM", task: "LinkedIn DM to Recruiter", detail: "After applying, find the HR/recruiter on LinkedIn. Send a short connection note mentioning SEVC AIR3 and Euler Motors — 1–2 DMs/day.", icon: "💬" },
  { time: "9:40 – 10:00 AM", task: "Log Everything", detail: "Add all applications to this tracker. Set follow-up reminders for 7 days later. Track your daily count — hit 10 before stopping.", icon: "✅" },
];

const SEARCH_TERMS = [
  "EV Design Engineer fresher","Wiring Harness Engineer fresher India",
  "CAD Design Engineer SolidWorks","PCB Design Engineer KiCAD",
  "Electrical Systems Engineer EV","Battery Engineer fresher",
  "Mechatronics Engineer fresher","Test Engineer embedded systems",
  "Junior Electrical Engineer automotive","ECAD Engineer AutoCAD Electrical",
];
...
const styles: Record<string, CSSProperties> = {
  app:        { minHeight: "100vh", background: "linear-gradient(135deg,#0F172A 0%,#1E293B 50%,#0F172A 100%)", fontFamily: "'DM Sans','Segoe UI',sans-serif", color: "#E2E8F0" },
  header:     { background: "linear-gradient(90deg,#1A5376 0%,#2E86AB 100%)", padding: "20px 24px 16px", borderBottom: "2px solid #2E86AB44" },
  headerTitle:{ fontSize: 22, fontWeight: 800, color: "#fff", margin: 0 },
  headerSub:  { fontSize: 13, color: "#BAE6FD", marginTop: 4 },
  statsRow:   { display: "flex", gap: 12, padding: "16px 24px", borderBottom: "1px solid #1E293B" },
  stat:       { flex: 1, background: "#1E293B", borderRadius: 12, padding: "14px 16px", textAlign: "center" as const, border: "1px solid #334155" },
  statNum:    { fontSize: 28, fontWeight: 800, color: "#38BDF8" },
  statLabel:  { fontSize: 11, color: "#94A3B8", marginTop: 2, textTransform: "uppercase" as const, letterSpacing: 1 },
  content:    { padding: "20px 24px", maxWidth: 900, margin: "0 auto" },
  tipBar:     { background: "#1A5376", borderRadius: 10, padding: "10px 16px", marginBottom: 16, fontSize: 13, color: "#BAE6FD", display: "flex", alignItems: "center", gap: 8, border: "1px solid #2E86AB44" },
  card:       { background: "#1E293B", borderRadius: 12, padding: "14px 16px", marginBottom: 10, border: "1px solid #334155", display: "flex", alignItems: "flex-start" as const, gap: 12 },
  input:      { background: "#0F172A", border: "1px solid #334155", borderRadius: 8, padding: "8px 12px", color: "#E2E8F0", fontSize: 13, width: "100%", boxSizing: "border-box" as const },
  select:     { background: "#0F172A", border: "1px solid #334155", borderRadius: 8, padding: "8px 12px", color: "#E2E8F0", fontSize: 13, width: "100%" },
  formGrid:   { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 10 },
  guideCard:  { background: "#1E293B", borderRadius: 12, padding: 16, marginBottom: 12, border: "1px solid #334155", display: "flex", gap: 14, alignItems: "flex-start" as const },
  timeChip:   { background: "#0F172A", color: "#38BDF8", borderRadius: 8, padding: "4px 10px", fontSize: 11, fontWeight: 700, whiteSpace: "nowrap" as const, border: "1px solid #1A5376" },
  searchTag:  { background: "#0F172A", border: "1px solid #2E86AB55", borderRadius: 20, padding: "5px 12px", fontSize: 12, color: "#7DD3FC", display: "inline-block", margin: "4px", cursor: "pointer" },
};

// ── Components ────────────────────────────────────────────────────────────────
function Tag({ label, color = "#3B82F6", bg = "#EFF6FF" }: TagProps) {
  return (
    <span style={{ background: bg, color, border: `1px solid ${color}33`, borderRadius: 6, padding: "2px 10px", fontSize: 12, fontWeight: 600 }}>
      {label}
    </span>
  );
}
...
export default function App() {
  ...
  const [form, setForm] = useState({ company: "", role: "", platform: "", status: "Applied", notes: "", date: new Date().toISOString().slice(0, 10) });
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem("gokularam-job-apps");
  ...
  useEffect(() => {
    try { localStorage.setItem("gokularam-job-apps", JSON.stringify(apps)); } catch (e) {}
  }, [apps]);
  ...
  const inReviewCount = apps.filter(a => a.status === "In Review").length;
  ...
  const btnStyle = (color = "#2E86AB"): CSSProperties => ({
    ...
    <div style={styles.app}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <div style={{ textAlign: "right" as const }}>
            ...
            <div style={{ background: "#0F172A", borderRadius: 20, height: 10, width: 140, overflow: "hidden" }}>
              <div style={{ background: progressPct >= 100 ? "#10B981" : "linear-gradient(90deg,#2E86AB,#38BDF8)", height: "100%", width: `${progressPct}%`, borderRadius: 20, transition: "width 0.5s" }} />
            </div>
            <div style={{ fontSize: 12, color: "#BAE6FD", marginTop: 4 }}>{todayCount}/10 today {todayCount >= 10 ? "✅" : ""}</div>
          </div>
        </div>
      </div>
      ...
      <div style={styles.statsRow}>
        {[
          ...
          { num: apps.length,     label: "Total Applied",     color: "#38BDF8" },
          { num: interviewCount,  label: "Interviews/Offers", color: "#10B981" },
          { num: inReviewCount,   label: "In Review",         color: "#F59E0B" },
        ].map((s, i) => (
      ...
      {/* Tabs */}
      <div style={{ display: "flex", gap: 0, padding: "0 24px", borderBottom: "1px solid #1E293B", background: "#0F172A" }}>
      ...
      <div style={styles.content}>
        {/* Tip */}
        <div style={styles.tipBar}><span>💡</span><span>{TIPS[tipIndex]}</span></div>
          ...
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14, flexWrap: "wrap" as const, gap: 8 }}>
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" as const }}>
                {["All", ...Object.keys(STATUS_CONFIG)].map(s => (
                  <button key={s} onClick={() => setFilterStatus(s)} style={{ ...btnStyle(filterStatus === s ? "#2E86AB" : "#1E293B"), border: "1px solid #334155", fontSize: 12, padding: "6px 12px" }}>{s}</button>
                ))}
              </div>
              <button style={btnStyle("#10B981")} onClick={() => setShowForm(f => !f)}>
            ...
            {showForm && (
              <div style={{ background: "#1E293B", borderRadius: 12, padding: 16, marginBottom: 16, border: "1px solid #2E86AB55" }}>
                ...
                <button style={btnStyle("#10B981")} onClick={addApp}>✓ Save Application</button>
              </div>
            )}

            {filtered.length === 0 ? (
              <div style={{ textAlign: "center" as const, color: "#475569", padding: 40 }}>
              ...
              <div key={app.id} style={styles.card}>
                ...
                <div style={{ display: "flex", flexDirection: "column" as const, gap: 4, minWidth: 90 }}>
                  ...
                  <button onClick={() => deleteApp(app.id)} style={{ background: "none", border: "1px solid #EF444433", color: "#EF4444", borderRadius: 6, padding: "3px 8px", fontSize: 11, cursor: "pointer" }}>Delete</button>
                </div>
              </div>
            ))}
          </div>
          ...
          <div>
            ...
            {DAILY_GUIDE.map((g, i) => (
              <div key={i} style={styles.guideCard}>
                ...
                <div style={{ background: "#0F172A", color: "#38BDF8", width: 28, height: 28, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 800, border: "2px solid #2E86AB", flexShrink: 0 }}>{i + 1}</div>
              </div>
            ))}
        ...
        {tab === "search" && (
          <div>
            <p style={{ color: "#94A3B8", marginBottom: 12, fontSize: 13 }}>
              Copy these into <strong style={{ color: "#38BDF8" }}>LinkedIn Jobs</strong> and <strong style={{ color: "#38BDF8" }}>Naukri</strong> every morning.
            </p>
            <div style={{ marginBottom: 20 }}>
              {SEARCH_TERMS.map(t => (
                <span key={t} style={styles.searchTag} onClick={() => navigator.clipboard?.writeText(t)} title="Click to copy">🔍 {t}</span>
              ))}
            </div>
            ...
            {[
              ...
              { role: "CAD / Design",    companies: ["Tata Elxsi","KPIT","QuEST Global","Cyient","L&T Technology Services"] },
              ...
              <div key={role} style={{ marginBottom: 14 }}>
                <div style={{ fontSize: 12, color: "#F59E0B", fontWeight: 700, marginBottom: 6, textTransform: "uppercase" as const, letterSpacing: 1 }}>{role}</div>
                <div style={{ display: "flex", flexWrap: "wrap" as const, gap: 6 }}>
                  {companies.map(c => (
                    <span key={c} style={{ background: "#1E293B", border: "1px solid #334155", borderRadius: 20, padding: "4px 12px", fontSize: 12, color: "#CBD5E1" }}>{c}</span>
                  ))}
                </div>
              </div>
            ))}
            <div style={{ background: "#1E293B", borderRadius: 12, padding: 16, marginTop: 8, border: "1px solid #334155" }}>
              ...
              <div style={{ background: "#0F172A", borderRadius: 8, padding: 14, fontSize: 13, color: "#94A3B8", lineHeight: 1.7, border: "1px solid #1E293B", fontStyle: "italic" as const }}>
                "Hi [Name], I just applied for the [Role] position at [Company] and wanted to connect directly. I'm a final-year EEE graduate with hands-on experience in [relevant skill], currently wrapping up an EV battery industrialization internship at Euler Motors. I also achieved All India Rank 3 in EV Design at SEVC 2025. Would love to be considered!"
              </div>
              <p style={{ fontSize: 11, color: "#475569", marginTop: 8 }}>✏️ Customize [Name], [Role], [Company], and [relevant skill] before sending.</p>
            </div>
          </div>
        )}
      </div>
